...
- (NSDictionary*)resultDict {
    ...
    if (num_cols > 0) {
        ...
        [self log:SFLogLevelDebug msg:@"Warning: There seem to be no columns in this set."];
    }
    ...
    do {
        ...
        if (SQLITE_BUSY == rc || SQLITE_LOCKED == rc) {
            ...
            if (SQLITE_LOCKED == rc) {
                ...
                if (rc != SQLITE_LOCKED) {
                    [self log:SFLogLevelDebug format:@"Unexpected result from sqlite3_reset (%d) rs", rc];
                }
            }
            usleep(20);
            ...
            [self log:SFLogLevelDebug format:@"Error calling sqlite3_step (%d: %s) rs", rc, sqlite3_errmsg([_parentDB sqliteHandle])];
            break;
        } 
...
- (int)columnIndexForName:(NSString*)columnName {
    ...
    [self log:SFLogLevelDebug format:@"Warning: I could not find the column named '%@'.", columnName];
