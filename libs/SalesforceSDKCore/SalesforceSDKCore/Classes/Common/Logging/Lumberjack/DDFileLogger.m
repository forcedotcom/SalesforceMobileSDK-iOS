...
- (instancetype)initWithLogsDirectory:(NSString *)aLogsDirectory
{
    if ((self = [super init]))
    {
        ...
        NSString *keyPath = NSStringFromSelector(@selector(maximumNumberOfLogFiles));
        if ([self respondsToSelector:NSSelectorFromString(keyPath)]) {
            [self addObserver:self forKeyPath:keyPath options:kvoOptions context:nil];
        }
...
+ (BOOL)automaticallyNotifiesObserversForKey:(NSString *)theKey {
    ...
    BOOL automatic = NO;
    if ([theKey isEqualToString:@"maximumNumberOfLogFiles"]) {
        automatic = NO;
    }
    else {
...
- (void)deleteOldLogFiles
{
    ...
    for (i = maxNumLogFiles; i < count; i++)
    {
        ...
        [fileManager removeItemAtPath:logFileInfo.filePath error:nil];
    }
}
...
- (NSString *)logsDirectory
{
    ...
    NSFileManager *fileManager = [[NSFileManager alloc] init];
    if (![fileManager fileExistsAtPath:_logsDirectory])
    {
        NSError *err = nil;
        if (![fileManager createDirectoryAtPath:_logsDirectory
...
- (NSDictionary *)fileAttributes
{
    ...
    {
        NSFileManager *fileManager = [[NSFileManager alloc] init];
        fileAttributes = [fileManager attributesOfItemAtPath:filePath error:nil];
    }
